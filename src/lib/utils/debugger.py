...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    if dataset == 'coco_hp':
      ...
      self.colors_hp = [(255, 0, 255), (255, 0, 0), (0, 0, 255), 
        ...
        self.names = ['p']
        self.colors = {'Car': (255, 0, 255), 'Pedestrian': (0, 255, 255), 'Cyclist':(255, 0, 0)}
        self.num_class = 1
        self.num_joints = 9
  ...
  def add_bev(self, box,img_id, color=(255,0,0)):
      position=box[11:14]
  ...
  def save_kitti_formet(self, results, img_path, opt,img_id='default'):
    # points=points_dim[:16]
    # dim = points_dim[16:19]
    # points = np.array(points, dtype=np.int32).reshape(self.num_joints, 2)
    ...
    pos = results[11:14]
    ori = results[14]
    self.write_detection_results(results[0],result_dir,file_number,box,dim,pos,ori,score)
  ...
  def add_3d_detection(self, results, calib, img_id='default', show_txt=False):
      ...
      self.imgs[img_id] = draw_box_3d(self.imgs[img_id], box_2d, cl, cat, conf)

...
def car_pose_post_process(dets, c, s, h, w):
  # dets: batch x max_dets x 40
  # return list of 39 in image coords cores,kps_l,kps_r,rot_y
  ret = []
  for i in range(dets.shape[0]):
    pts_l = dets[i, :, 9:27]#transform_preds(dets[i, :, 5:23].reshape(-1, 2), c[i], s[i], (w, h))
    pts_r = dets[i, :, 27:45]

    top_preds = np.concatenate(
      [dets[i, :, 0:4],dets[i, :, 4:8],dets[i, :, 8:9],
       pts_l,pts_r,dets[i, :, 45:46]], axis=1).astype(np.float32).tolist()

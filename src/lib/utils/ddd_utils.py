...
def draw_box_3d(image, corners, c=(0, 0, 255), cat = None, conf = 0):
  face_idx = [[0,1,5,4],
              [1,2,6, 5],
              [2,3,7,6],
              [3,0,4,7]]
  box_min = np.min(corners, axis = 0)#.astype(np.uint8)
  box_max = np.max(corners, axis = 0)#.astype(np.uint8)
  box_min = box_min.astype(np.int)
  box_max = box_max.astype(np.int)
  for ind_f in range(3, -1, -1):
    ...
    if ind_f == 0:
      cv2.line(image, (corners[f[0], 0], corners[f[0], 1]),
               (corners[f[2], 0], corners[f[2], 1]), c, 2, lineType=cv2.LINE_AA)
      cv2.line(image, (corners[f[1], 0], corners[f[1], 1]),
               (corners[f[3], 0], corners[f[3], 1]), c, 2, lineType=cv2.LINE_AA)
  ...
  cv2.rectangle(image,(box_min[0], box_min[1] - cat_size[1] - 2),
                  (box_min[0] + cat_size[0], box_min[1] - 2), c, -1)
  cv2.putText(image, txt, (box_min[0], box_min[1] - 2),
